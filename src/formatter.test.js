...
const fmtBytes = (input, opts) =>
  decoder.decode(formatBytes(encoder.encode(input), opts).output);

/** Reference formatter (uses JSON.parse → JSON.stringify). */
const reference = (input, indent = 2) =>
...
describe("Objects", () => {
  ...
  it("formats single-key object", () =>
    assertMatchesReference('{"a":1}'));

  it("formats multi-key object", () =>
    assertMatchesReference('{"a":1,"b":2,"c":3}'));

  it("formats object with string values", () =>
    assertMatchesReference('{"name":"Alice","city":"NYC"}'));
...
describe("Arrays", () => {
  ...
  it("formats single-element array", () =>
    assertMatchesReference("[1]"));

  it("formats multi-element array", () =>
    assertMatchesReference("[1,2,3]"));

  it("formats array of strings", () =>
    assertMatchesReference('["a","b","c"]'));

  it("formats array of mixed types", () =>
    assertMatchesReference('[1,"two",true,null]'));
});
...
describe("Nesting", () => {
  it("formats nested objects", () =>
    assertMatchesReference('{"a":{"b":{"c":1}}}'));

  it("formats nested arrays", () =>
    assertMatchesReference("[[[1,2],[3,4]],[[5]]]"));

  it("formats objects inside arrays", () =>
    assertMatchesReference('[{"a":1},{"b":2}]'));

  it("formats arrays inside objects", () =>
    assertMatchesReference('{"list":[1,2,3],"nested":{"deep":[4,5]}}'));
...
describe("Strings and escaping", () => {
  it("preserves escaped quotes", () =>
    assertMatchesReference('{"say":"he said \\"hi\\""}'));

  it("preserves escaped backslashes", () =>
    assertMatchesReference('{"path":"c:\\\\users\\\\file"}'));

  it("preserves escaped newlines", () =>
    assertMatchesReference('{"text":"line1\\nline2"}'));
...
describe("Whitespace handling", () => {
  it("formats already-minified JSON", () =>
    assertMatchesReference('{"a":1,"b":[2,3]}'));
...
describe("Edge cases", () => {
  it("formats empty object with whitespace", () =>
    assertMatchesReference("{  }"));

  it("formats empty array with whitespace", () =>
    assertMatchesReference("[  ]"));
  ...
  it("handles empty nested containers", () =>
    assertMatchesReference('{"a":{},"b":[],"c":{"d":[]}}'));
